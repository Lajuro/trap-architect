...
const SORT_OPTIONS = [
  { value: "created_at", labelKey: "sortRecent" },
  { value: "plays", labelKey: "sortMostPlayed" },
  { value: "likes", labelKey: "sortMostLiked" },
  { value: "difficulty", labelKey: "sortHardest" },
] as const;

const DIFFICULTY_FILTERS = [
  { value: "", labelKey: "all", color: "#888" },
  { value: "easy", labelKey: "easy", color: "#22C55E" },
  { value: "medium", labelKey: "medium", color: "#EAB308" },
  { value: "hard", labelKey: "hard", color: "#F97316" },
  { value: "extreme", labelKey: "extreme", color: "#EF4444" },
] as const;
...
export default function BrowsePage() {
  ...
  const fetchLevels = useCallback(
    ...
    [sort, debouncedSearch, difficultyFilter, tagFilter],
  );
    ...
    <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full overflow-y-auto">
          ...
          <div className="flex gap-2 flex-wrap">
            {DIFFICULTY_FILTERS.map((df) => (
              <button
                ...
                aria-pressed={difficultyFilter === df.value}
              ...
              >
                {df.value === "" ? tc("all") : td(df.labelKey)}
              </button>
            ))}
          </div>
        ...
        {loading && levels.length === 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" aria-busy="true">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-muted/30 border-2 border-border animate-pulse">
          ...
          <HudPanel className="text-center py-16">
            ...
            <p className="text-[10px] mt-4 mb-2">{t("noLevelsFound")}</p>
            ...
            {hasMore && (
              <div className="text-center mt-6">
                <HudButton
                  ...
                  variant="secondary"
                >
                  {loading ? tc("loadingEllipsis") : tc("loadMore")}
                </HudButton>
              </div>
            )}
          </>
