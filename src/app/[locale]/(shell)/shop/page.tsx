...
export default function ShopPage() {
  ...
  const tSkins = useTranslations("cosmetics.skins");
  const tTrails = useTranslations("cosmetics.trails");
  const tDeaths = useTranslations("cosmetics.deathEffects");
  const tFrames = useTranslations("cosmetics.frames");
  ...
  async function equipItem(field: string, value: string) {
    ...
    setMessage(t("equippedMsg"));
  }

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-muted-foreground text-[9px] uppercase tracking-wider">{tc("loadingEllipsis")}</p>
      </div>
    );
  }

  const tabs: { id: typeof tab; labelKey: string; icon: PixelIconName }[] = [
    { id: "skins", labelKey: "tabSkins" as const, icon: "paint" },
    { id: "trails", labelKey: "tabTrails" as const, icon: "sparkle" },
    { id: "deaths", labelKey: "tabDeaths" as const, icon: "skull" },
    { id: "frames", labelKey: "tabFrames" as const, icon: "crown" },
  ];
  ...
  function renderCosmeticCard(
    ...
    return (
      ...
      >
        ...
        {equipped ? (
          <span className="text-[8px] text-primary font-medium uppercase tracking-wider">{tc("equipped")}</span>
        ) : owned ? (
          <HudButton onClick={onEquip} variant="secondary" size="small">{tc("equip")}</HudButton>
        ) : (
    ...
    <main className="flex-1 max-w-5xl mx-auto px-4 py-6 w-full overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-[11px] font-bold uppercase tracking-wider flex items-center gap-2">
                <PixelIcon name="shop" size={16} color="#FFD700" /> {t("title")}
              </h1>
        ...
        <div className="flex gap-1 mb-6 border-b-2 border-border">
          {tabs.map((tabItem) => (
            ...
            >
              <PixelIcon name={tabItem.icon} size={10} /> {t(tabItem.labelKey)}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {SKINS.map((skin) => {
              ...
              return renderCosmeticCard(
                skin.id, tSkins(skin.id), skin.cost, owned, equipped,
        ...
        {tab === "trails" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {TRAILS.map((trail) => {
              ...
              return renderCosmeticCard(
                trail.id, tTrails(trail.id), trail.cost, owned, equipped,
        ...
        {tab === "deaths" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {DEATH_EFFECTS.map((effect) => {
              ...
              return renderCosmeticCard(
                effect.id, tDeaths(effect.id), effect.cost, owned, equipped,
        ...
        {tab === "frames" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {FRAMES.map((frame) => {
              ...
              return renderCosmeticCard(
                frame.id, tFrames(frame.id), frame.cost, owned, equipped,
