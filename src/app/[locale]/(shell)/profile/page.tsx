      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 py-6 w-full overflow-y-auto">
        <HudPanel className="mb-6">
          <h1 className="text-[11px] font-bold uppercase tracking-wider flex items-center gap-2 mb-1">
            <PixelIcon name="profile" size={16} /> {t("title")}
          </h1>
        </HudPanel>
        ...
        {profile && (
          <div className="grid grid-cols-3 gap-3 mb-6">
            <HudPanel className="text-center">
              ...
              <p className="text-[8px] text-muted-foreground uppercase tracking-wider">{tc("levels")}</p>
            </HudPanel>
            <HudPanel className="text-center">
        ...
        <HudPanel className="mb-6">
          <h2 className="text-[10px] font-bold mb-4 uppercase tracking-wider">{t("info")}</h2>
        ...
        <div className="flex flex-wrap gap-3">
          <HudButton href="/editor" variant="primary">
            <PixelIcon name="create" size={12} /> {t("createLevel")}
          </HudButton>
          <HudButton href="/play" variant="secondary">
            <PixelIcon name="play" size={12} /> {t("campaignBtn")}
          </HudButton>
          <HudButton href="/shop" variant="gold">
            <PixelIcon name="shop" size={12} /> {t("shopBtn")}
          </HudButton>
          <HudButton href="/settings" variant="ghost">
            <PixelIcon name="settings" size={12} /> {t("configBtn")}
          </HudButton>
          <HudButton onClick={handleLogout} variant="ghost">
            <PixelIcon name="logout" size={12} /> {t("logout")}
          </HudButton>
        </div>
        ...
        <HudPanel className="mt-6">
          <h2 className="text-[10px] font-bold mb-4 uppercase tracking-wider flex items-center gap-2">
            <PixelIcon name="trophy" size={14} color="#FFD700" /> {t("achievements")} ({achievements.length}/{ACHIEVEMENTS.length})
          </h2>
          {newAchievements.length > 0 && (
            <HudPanel variant="highlight" className="mb-4">
              <p className="text-[9px] font-bold text-yellow-400 mb-1">{t("newAchievements")}</p>
              {newAchievements.map((id) => {
                const def = ACHIEVEMENTS.find((a) => a.id === id);
                return def ? (
                  <p key={id} className="text-[8px]">
                    {def.icon} {tAch(`${def.id}.name`)} — {tAch(`${def.id}.description`)}
                  </p>
                ) : null;
              })}
            </HudPanel>
