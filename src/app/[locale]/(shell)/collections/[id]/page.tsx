  ...
  if (error || !collection) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4">
        <p className="text-[10px] text-muted-foreground">{error || t("notFound")}</p>
        <Link href="/collections">
          <HudButton variant="ghost" size="small">{tc("back")}</HudButton>
        </Link>
      </div>
    );
  }
    ...
    <main className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full overflow-y-auto">
        ...
        <HudPanel className="mb-6">
          ...
          <div className="flex items-center gap-3 mt-3 text-[8px] text-muted-foreground">
            <span>{tc("by")} {collection.profiles?.nickname || tc("anonymous")}</span>
            <span>{t("levelCount", { count: collection.levels?.length || 0 })}</span>
        ...
        {!collection.levels || collection.levels.length === 0 ? (
          <HudPanel className="text-center py-16">
            <p className="text-[10px]">{t("emptyCollection")}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {collection.levels.map((level) => (
              <LevelCard
                ...
                id={level.id}
                name={level.title}
                authorName={level.profiles?.nickname || tc("anonymous")}
